...
def _history_item_in_scope(callbacks, helpers, item):
    """Burp Target scopeの判定。URLを解決できない通信はScope対象にしない。"""
    try:
        return bool(callbacks.isInScope(helpers.analyzeRequest(item).getUrl()))
    except Exception:
        # Scope filterを選択した以上、判定不能な通信は安全側で除外する。
        return False
...
def analyze_history(callbacks, helpers, start_packet_no=None, end_packet_no=None, cancel_check=None,
    ...
    for item in callbacks.getProxyHistory():
        ...
        if end_packet_no is not None and packet_no > end_packet_no:
            break
        if scope_only and not _history_item_in_scope(callbacks, helpers, item):
            continue
...
def analyze_history_v2(callbacks, helpers, start_packet_no=None, end_packet_no=None, cancel_check=None,
    ...
    records = analyze_history(callbacks, helpers, start_packet_no, end_packet_no, cancel_check, scope_only)
    for record in records:
...
def summary_rows_v2(records):
    ...
    # Statistics画面では、分類行の合計を同じ表で確認できるよう末尾に置く。
    # including は解析対象の実件数、excluding は集約対象を除いたテスト候補数。
    rows.append({'traffic_class': u'', 'category': u'', 'protocol': u'Total',
                 'including_aggregated': sum(row['including_aggregated'] for row in rows),
                 'excluding_aggregated': sum(row['excluding_aggregated'] for row in rows),
                 'definition': u'表示中の分類行の合計。'})
    return rows
...
def clear_analysis_annotations(callbacks, start_packet_no=None, end_packet_no=None,
    ...
    for item in callbacks.getProxyHistory():
        ...
        if end_packet_no is not None and packet_no > end_packet_no:
            break
        if scope_only and (helpers is None or not _history_item_in_scope(callbacks, helpers, item)):
            continue
