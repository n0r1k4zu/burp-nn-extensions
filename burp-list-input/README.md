...
- **送信ログ** — 実際に送信されたリクエストと、返ってきたレスポンスの両方を確認できます。
...
- **History Search** — 指定したワードでProxy history全体（リクエスト・レスポンスの生バイト）を大文字小文字を区別せず検索し、ヒットした前後を指定文字数（既定30文字）だけ切り出して一覧表示します。同じパケット内に複数ヒットがあれば、それぞれ別の行として表示されます。行を選択すると、該当パケットのリクエスト/レスポンスをその場でプレビューでき、選択行の表示内容をそのまま**Decode**タブへ送って複数形式で同時にデコードすることもできます。
...
6. リストの下にある **Send selected row to Decode** を押すと、選択中の行の表示内容（Before + Match + After）が **Decode** タブに送られ、複数形式で同時にデコード・エンコードして確認できる
7. **Clear** で検索語・文字数指定・結果一覧をまとめてリセットできる
