...
- **Statisticsコメント付きExport** — HTTP Historyの右クリック **Export Packet & Insertion Point with Statistics comment** は、選択パケットだけへ現在のStatisticsと同じ`[Protocol=...] [Traffic Class=...] [Category=...]`と集約タグを追加してから、同じInsertion Point CSVを出力します。自動採番は行いません。出力成功後は、追加したタグを残すか、処理前のCommentへ復元するかを選べます。Auraの集約判定はHistory全体を参照しますが、Comment変更とCSV対象は選択パケットだけです。
...
- **選択HistoryのClear Comment / Clear Color** — HTTP Historyで1件以上のパケットを選択して右クリックすると、**Clear Comment** で選択パケットのCommentだけを削除、**Clear Color** で選択パケットの色だけを解除できます。未選択・他のHistoryパケットには影響しません。
...
Target & List Mappingは未ロード時にサンプルCSVを表示します。**Export CSV...**で現在のリスト（サンプル表示中はそのサンプル）を出力でき、**Clear list**で実データを消去します。Arm／Re-detectはActiveを自動で有効化しません。
