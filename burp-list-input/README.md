...
- **Target & Replace with Decode & Encode** — Target & List Mappingとは別に右クリックでarmした対象について、特定のInsertion Pointの値を書き換える機能です。値がURL/Base64/Hex/HTMLエンティティ/Unicodeエスケープ/ROT13でエンコードされていても、①デコード → ②指定した文字列（プレーン or 正規表現）で置換 → ③同じ方式で再エンコード、という流れを送信のたびに自動で行ってから送信します。Codecには単体に加え、`URL → Base64`のような2段の組み合わせもあり、左から外側→内側の順にデコードし、逆順に再エンコードします。エンコードされたパラメータの中身だけを書き換えたい場合に使います。Insertion Point一覧で行を選択すると、画面下部にOriginal Value（元の値）と、選択中のCodecでデコードした後の値がその場でプレビュー表示されます。実行結果は他の機能と同じくLogタブに記録されます。
