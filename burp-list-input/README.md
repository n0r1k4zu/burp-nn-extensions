...
- **My Word List / My Word List Grep** — `Word,Comment` のCSVをロード／編集／Exportし、リストの各WordをPacket Grepと同じ条件・範囲・プレビュー・Decode機能で一括検索します。結果の末尾にはMy Word ListのCommentを表示します。
- **Backup & Restore** — My Word List、Target & List MappingのCSV、Match & ReplaceのRequest／Responseルールを、編集可能なMarkdown内のCSVコードブロックとしてExport／Restoreします。Color SnapshotsとComment Snapshotsは対象外です。処理はバックグラウンドで行われ、実行中のボタン表示で進行状況を確認できます。
