...
> ⚠️ 短い/よくある文字列（例: 1文字の"e"等）をワードに指定すると、大きなレスポンス1件だけで数千〜数万件
> ヒットすることがあります。これによるBurp全体のフリーズを防ぐため、**1通信あたりのヒット数は200件まで**
> に制限しています（それを超えた分は記録されません）。実用上は、ある程度絞り込んだ具体的な文字列を
> ワードに指定することを推奨します。
